            ...
            <div x-show="open" x-collapse x-cloak>
                <div class="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 mt-4">
                    @foreach($upcomingTransits as $transit)
                        <div class="bg-slate-900/60 p-4 rounded-2xl border {{ $transit->type === 'sun' ? 'border-amber-500/20' : 'border-blue-500/20' }} flex flex-col justify-between">
                            <div>
                                <div class="flex justify-between items-start">
                                    <span class="text-slate-100 font-bold text-base">{{ \Carbon\Carbon::parse($transit->time)->timezone(config('app.timezone', 'UTC'))->format('l, M jS') }}</span>
