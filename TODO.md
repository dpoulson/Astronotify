...
- [ ] M **Test notification button** — send a test email from the location card to confirm delivery is working
- [x] H **Unsubscribe / re-subscribe link** in email footers so users can opt out without logging in

## 🛠️ Admin
- [ ] M **Fix stats / metrics** — verify system_metrics and daily_metrics are incrementing correctly; display graphs on admin dashboard
- [x] H **API usage tracker** — show total Open-Meteo calls this month vs free tier limit (1,000/day)
- [ ] M **Per-user location count** and last-active date in user table
- [x] H **Manual trigger buttons** in admin UI for `weather:fetch` and `weather:iss-transits` (instead of SSH + artisan)
- [ ] M **Failed job queue** — surface any queued email failures in the admin dashboard with retry button

## 📄 Pages
- [ ] M **Update About page** — reflect current feature set (ISS transits, elevation support, solar/lunar path diagrams, etc.)
...
## 🗺️ Dashboard UX
- [ ] M **Fix jerk on Add Location expand/collapse** — the sidebar width transition causes a layout reflow when scrolled down; investigate whether `position: sticky` or `overflow: hidden` on the parent resolves it without breaking the layout
...
- [ ] M **Conjunction threshold setting** — currently hard-coded at 0.75°; expose this in the admin settings page
...
- [ ] M **Moon phase display** — show moon phase icon on each forecast day card (affects naked-eye astronomy)
- [ ] M **Light pollution overlay** — link to or embed a Bortle scale indicator for each location
